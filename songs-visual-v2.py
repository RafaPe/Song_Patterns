...
if not cluster_hide:
    ...
    if songs_from == 'Fer Songs':
        ...
        if cluster == 'Cluster1':
            ...
            d = 'Danceability mean: ' + str(sample['danceability'].mean()) + ' standard deviation: ' + str(sample['danceability'].std())
            st.markdown(d)
            st.markdown(sample['energy'].mean())
            st.markdown(sample['key'].mean())
            st.markdown(sample['loudness'].mean())
            st.markdown(sample['mode'].mean())
            st.markdown(sample['valence'].mean())
            st.markdown(sample['tempo'].mean())
            st.markdown(stat.mode(sample.country))
            st.markdown(stat.mode(sample.song_language))
            st.write(sample)
        ...
        if cluster == 'Cluster2':
            ...
            st.markdown(sample['danceability'].mean())
            st.markdown(sample['energy'].mean())
            st.markdown(sample['key'].mean())
            st.markdown(sample['loudness'].mean())
            st.markdown(sample['mode'].mean())
